...
const TherapistDashboard = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setStats({
        ...
        balance: Number(wallet.data?.available_balance ?? wallet.data?.balance ?? 0),
  ...
  const cards = [
    ...
    { label: "Sessions This Month", value: stats.bookings, icon: Calendar },
    { label: "Earnings", value: `$${stats.balance.toFixed(2)}`, icon: Wallet },
    { label: "Rating", value: stats.rating > 0 ? stats.rating.toFixed(1) : "N/A", icon: Star },
  ];

  return (
    <DashboardLayout role="therapist">
      <h1 className="text-2xl font-bold text-foreground mb-2">Therapist Dashboard</h1>
      <p className="text-muted-foreground text-sm mb-8">Welcome back, {profile?.full_name || "Therapist"}</p>
