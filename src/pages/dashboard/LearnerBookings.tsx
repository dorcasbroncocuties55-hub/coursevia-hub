...
const LearnerBookings = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("bookings").select("*, coach_profiles(*, profiles(*))").eq("learner_id", user.id).order("created_at", { ascending: false }).then(({ data }) => {
      setBookings(data || []);
    });
  }, [user]);
    ...
    <DashboardLayout role="learner">
      ...
      {bookings.length === 0 ? (
        <div className="bg-card border border-border rounded-lg p-8 text-center">
          ...
          <a href="/coaches" className="text-primary hover:underline text-sm mt-2 inline-block">Find a coach</a>
        </div>
      ) : (
        <div className="space-y-3">
          {bookings.map((b) => (
            <div key={b.id} className="bg-card border border-border rounded-lg p-4 flex items-center justify-between">
              <div>
                <p className="font-medium text-foreground">{b.notes || "Coaching session"}</p>
